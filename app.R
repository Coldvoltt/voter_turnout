...
ui <- dashboardPage(skin = "blue",
      ...
      https://www.idea.int/data-tools/vt-advanced-search?region=&question=")
                    ...
                    dashboardBody(
                      ...
                      fluidRow(
                        box(title = "Voter's population for each country", solidHeader = T,
                            width = 7,  collapsible = T,
                            plotlyOutput("plot3")),
...
server <- function(input, output, session) {
  ...
  # Read the CSV file during app initialization
  data <- read.csv('world_voter_turnout_data.csv', header = TRUE)
  ...
  output$box_A <- renderValueBox({
    ...
    valueBox(paste0(av_voter_turnout_pre[,3], "%"), 
             "Presidential", icon = icon("certificate"), color = 'aqua')
  })
  ...
  output$box_B <- renderValueBox({
    ...
    valueBox(paste0(av_voter_turnout_par[,3], "%"), 
             "Parliamentary", icon = icon("institution"), color = "blue")
  })
  ...
  output$box_C <- renderValueBox({
    ...
    valueBox(pop[1,3],
             "Nation's Population", icon = icon("globe"), color = "navy")
  })
